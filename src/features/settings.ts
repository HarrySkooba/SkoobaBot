...
export async function handleSettingsCommand(interaction: ChatInputCommandInteraction): Promise<boolean> {
  ...
  if (interaction.commandName === 'role-rule') {
    ...
    audit(
      ...
      'role_rule.updated',
      { scenario, checkRoleId: checkRole?.id ?? null, grantRoleId: grantRole?.id ?? null },
      interaction.user.id,
    );
...
export async function handleSettingsButton(interaction: ButtonInteraction): Promise<boolean> {
...
async function ensureAdminPanelChannel(guild: Guild, actorId: string) {
...
async function postAdminPanel(channel: Awaited<ReturnType<typeof ensureAdminPanelChannel>>) {
  const embed = new EmbedBuilder()
    ...
    .addFields(
      { name: 'Настройки', value: '`/setting set` и кнопка ниже покажут текущие значения.' },
      { name: 'Правила ролей', value: '`/role-rule` задает проверяемую и выдаваемую роль для сценариев.' },
    )
    .setColor(0x2b2d31);

  const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder().setCustomId('settings:summary').setLabel('Показать настройки').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('settings:help').setLabel('Как настраивать').setStyle(ButtonStyle.Primary),
  );
