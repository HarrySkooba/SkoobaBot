  ...
  await interaction.channel.send({
    embeds: [
      new EmbedBuilder()
        ...
        .setDescription(
          'Если после принятия заявки или сбоя бота у тебя нет нужной роли, нажми кнопку ниже. Выдается только отсутствующая роль.',
        )
        .setColor(0x5865f2),
    ],
...
export async function handleRoleRecoveryButton(interaction: ButtonInteraction): Promise<boolean> {
  ...
  const settingKey: SettingKey = roleKey === 'family' ? 'family_role_id' : 'unverified_role_id';
...
function buildRoleRecoveryButtons(): ActionRowBuilder<ButtonBuilder> {
  return new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder().setCustomId('roles:family').setLabel('Получить роль семьи').setStyle(ButtonStyle.Primary),
    new ButtonBuilder().setCustomId('roles:unverified').setLabel('Получить Unverified').setStyle(ButtonStyle.Secondary),
  );
}
