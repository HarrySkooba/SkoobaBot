...
const ACTIVE_CHEAT_STATUSES = ['waiting', 'called'] as const;
...
export async function handleCheatButton(interaction: ButtonInteraction): Promise<boolean> {
  ...
  if (!ACTIVE_CHEAT_STATUSES.includes(check.status as (typeof ACTIVE_CHEAT_STATUSES)[number])) {
...
export async function handleCheatModal(interaction: ModalSubmitInteraction): Promise<boolean> {
...
function getCheatCheck(checkId: number, guildId: string): CheatCheckRow | null {
  return (
    (db.prepare('SELECT id, guild_id, user_id, status, hunter_id, queue_message_id, result FROM cheat_checks WHERE id = ? AND guild_id = ?').get(checkId, guildId) as
      | CheatCheckRow
      | undefined) ?? null
  );
}

function cheatModalRow(customId: string, label: string, style: TextInputStyle) {
  return new ActionRowBuilder<TextInputBuilder>().addComponents(
    new TextInputBuilder().setCustomId(customId).setLabel(label).setStyle(style).setRequired(true).setMaxLength(1000),
  );
}

function cheatCheckQueueStatusLabel(status: string): string {
  switch (status) {
    case 'waiting':
      return 'Ожидание';
    case 'called':
...
function cheatCheckEmbedColor(status: string): number {
  switch (status) {
    case 'waiting':
      return 0xe67e22;
    case 'called':
...
function buildCheatCheckEmbed(check: Pick<CheatCheckRow, 'id' | 'user_id' | 'status' | 'hunter_id'>, options?: { rejectReason?: string }): EmbedBuilder {
...
function buildCheatCheckQueueContent(guildId: string, checkId: number, status: string): string {
  if (status === 'waiting') {
...
function cheatButtons(checkId: number, status: string) {
  const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder().setCustomId(`cheat:call:${checkId}`).setLabel('Вызвать').setStyle(ButtonStyle.Primary).setDisabled(status !== 'waiting'),
...
async function updateCheatQueueMessage(guild: Guild, check: CheatCheckRow, options?: { rejectReason?: string }) {
  ...
  const isActive = ACTIVE_CHEAT_STATUSES.includes(check.status as (typeof ACTIVE_CHEAT_STATUSES)[number]);
  await message
    .edit({
...
async function callPlayer(interaction: ButtonInteraction, check: CheatCheckRow) {
  if (!interaction.guild) {
    return;
  }
  ...
  db.prepare('UPDATE cheat_checks SET status = ?, hunter_id = ?, called_at = unixepoch(), updated_at = unixepoch() WHERE id = ?').run(
    'called',
    interaction.user.id,
    check.id,
  );

  const updatedCheck: CheatCheckRow = { ...check, status: 'called', hunter_id: interaction.user.id };
...
async function resolveCheck(
  ...
  const resolvedCheck: CheatCheckRow = { ...check, status, hunter_id: interaction.user.id, result };
    ...
    if (player) {
      ...
      if (failedRemoveRoleIds.length > 0) {
        await sendToConfiguredChannel(
          ...
          `Проверка #${check.id}: не удалось снять роли ${failedRemoveRoleIds.map((roleId) => `<@&${roleId}>`).join(', ')} с <@${player.id}>. Проверь, что роль бота выше этих ролей.`,
        );
      }
  ...
  await sendToConfiguredChannel(
    interaction.guild,
    'cheat_log_channel_id',
    `Проверка #${check.id}: ${statusLabel}${rejectReason ? ` (${rejectReason})` : ''}, игрок <@${check.user_id}>, CheatHunter <@${interaction.user.id}>.`,
  );
...
async function removePlayerFromQueue(interaction: ChatInputCommandInteraction) {
  ...
  const row = db
    ...
    .get(interaction.guild.id, player.id) as CheatCheckRow | undefined;
  ...
  const resolvedCheck: CheatCheckRow = { ...row, status: 'removed', hunter_id: interaction.user.id, result: reason };
