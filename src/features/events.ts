...
type EventRow = {
  ...
  list_closed: number;
  mcl_subtype: string | null;
  teleport_time: string | null;
...
type CreateEventInput = {
  ...
  mclSubtype?: string | null;
  teleportTime?: string | null;
...
export async function handleEventButton(interaction: ButtonInteraction): Promise<boolean> {
  ...
  if (action === 'signup' || action === 'leave') {
    ...
    if (action === 'signup') {
      ...
      db.prepare(
        `INSERT INTO event_signups (event_id, user_id, list_type, tier, promoted_from_reserve)
         VALUES (?, ?, ?, ?, 0)
         ON CONFLICT(event_id, user_id) DO UPDATE SET
           list_type = excluded.list_type,
           tier = excluded.tier,
           promoted_from_reserve = 0`,
      ).run(eventId, interaction.user.id, listType, tier);
      await refreshEventMessage(interaction.guild, eventId);
      const listLabel =
        listType === 'main' ? `основной список (тир ${tier})` : `запасной список${placement.listType === 'main' ? ', основной был полон' : ''}`;
  ...
  if (action === 'close-list') {
    ...
    await sendToConfiguredChannel(
      ...
      `МП #${eventId}: запись ${nextClosed ? 'закрыта' : 'открыта'} админом <@${interaction.user.id}>.`,
    );
...
function resolveEventVoiceChannelId(guildId: string, selectedVoiceChannelId: string | null): string | null {
  return selectedVoiceChannelId ?? getSetting(guildId, 'default_voice_channel_id');
}

function getEventTypeLabel(event: EventRow): string {
  if (event.type === 'kapt') {
    return 'Капт';
  }
...
function buildEventAnnounceContent(event: EventRow, familyRoleId: string): string {
...
async function createAndPublishEvent(
  ...
  createdBy: string,
  input: CreateEventInput,
): Promise<{ eventId: number; messageUrl: string } | null> {
  const result = db
    .prepare(
      `INSERT INTO events (
    ...
    .run(
      ...
      createdBy,
      input.mclSubtype ?? null,
      input.teleportTime ?? null,
...
async function sendEventMessage(channel: TextChannel, eventId: number, guild: Guild, options?: { announce?: boolean }) {
...
function buildEventMessagePayload(eventId: number, guild: Guild, options?: { announce?: boolean }) {
  const event = getEvent(eventId, guild.id);
  if (!event) {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      ...mainListChunks.map((chunk, index) => ({
        name:
          mainListChunks.length > 1
  ...
  const components = [
    ...
    new ActionRowBuilder<ButtonBuilder>().addComponents(
      ...
      new ButtonBuilder()
        ...
        .setStyle(listClosed ? ButtonStyle.Success : ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId(`event:delete:${eventId}`).setLabel('Удалить МП').setStyle(ButtonStyle.Danger),
    ),
  ];
  ...
  const payload: {
    content?: string;
    allowedMentions?: { roles: string[] } | { parse: [] };
    embeds: EmbedBuilder[];
...
function getEventSignupRows(eventId: number): EventSignupRow[] {
  return db
    .prepare(
      `SELECT user_id, list_type, tier, COALESCE(promoted_from_reserve, 0) AS promoted_from_reserve, created_at
...
function countMainSignups(eventId: number): number {
...
function chunkEmbedFieldLines(lines: string[], maxLength = 1024): string[] {
  if (!lines.length) {
    return ['Пусто'];
  }
...
function formatMainList(entries: EventMainEntry[]): string[] {
  if (!entries.length) {
    return ['Пусто'];
  }
...
function formatReserveList(userIds: string[]): string[] {
  if (!userIds.length) {
    return ['Пусто'];
  }
...
async function showPromoteReserveMenu(interaction: ButtonInteraction, event: EventRow) {
  if (!interaction.guild) {
    return;
  }
  ...
  const options = await Promise.all(
    lists.reserve.slice(0, 25).map(async (userId, index) => {
...
async function deleteEventMessage(interaction: ButtonInteraction, event: EventRow) {
  if (!interaction.guild) {
  ...
  await sendToConfiguredChannel(
    interaction.guild,
    getEventLogChannelKey(event.type),
    `МП #${event.id} удалено из канала админом <@${interaction.user.id}>. Сообщение убрано, данные и списки сохранены.\n${summary}`,
  );
