...
async function deletePlayerProfile(interaction, userId, reason) {
    ...
    audit(interaction.guild.id, 'profile.deleted', { profileId: profile.id, channelId: profile.channel_id, tier: profile.tier, channelDeleted, rolesRemoved, reason }, interaction.user.id, userId);
    await sendToConfiguredChannel(interaction.guild, 'profile_log_channel_id', `Профиль <@${userId}> удален админом <@${interaction.user.id}>. Тир был ${profile.tier}. Канал: ${channelDeleted ? 'удален' : 'не найден или не удален'}. Причина: ${reason}`);
    const details = [
